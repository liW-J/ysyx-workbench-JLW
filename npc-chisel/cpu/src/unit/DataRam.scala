...
class DataRamIO extends Bundle {
  ...
  val resALU               = Input(UInt(DATA_WIDTH.W))
...
class DataRam extends Module {
  ...
  dataLoad := dataMem.read(io.resALU >> DATA_BYTE_WIDTH_LOG.U)

  // STORE
  when(io.bundleDataControl.isStore) {
    when(io.bundleDataControl.lsType === LS_W) { // Store 4 bit
      dataMem.write(io.resALU >> DATA_BYTE_WIDTH_LOG.U, io.dataStore)
    }.elsewhen(io.bundleDataControl.lsType === LS_H) { // Store low 2 bit
      dataMem.write(io.resALU >> DATA_BYTE_WIDTH_LOG.U, Cat(dataLoad(31, 16), io.dataStore(15, 0)))
    }.otherwise { // Store low 1 bit
      dataMem.write(io.resALU >> DATA_BYTE_WIDTH_LOG.U, Cat(dataLoad(31, 8), io.dataStore(7, 0)))
    }
  }
